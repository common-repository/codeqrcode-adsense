...
Plugin URI: https://www.codeqrcode.com/
...
Version: 2.1.1
Author: Aklamator
Author URI: https://www.codeqrcode.com/
License: GPL2
...
if( !function_exists("codeQRCode_plugin_meta_links")) {
    function codeQRCode_plugin_meta_links($links, $file)
    {
        ...
        if ($file == $plugin) {
            return array_merge(
                $links,
                array('<a href="https://wordpress.org/support/plugin/codeqrcode-adsense/reviews" target=_blank>Please rate and review</a>')
            );
        }
        return $links;
    }
    ...
    function codeQRCode_bottom_post($content)
    {
        ...
        if (is_single()) {
            ...
            if(get_option('enableQROnSingle') != ''){
                ...
                }elseif(get_option('codeQRCodeSingle') != ""){ // User is selected some dynamic qr code
            ...
            if(get_option('enableQROnPage') != ''){
                ...
                }elseif(get_option('codeQRCodePage') != ""){ // User is selected some dynamic qr code
...
function show_qr_code_img($type, $img_size, $align, $margin, $called_from = ""){
    ...
    if($type == 'static'){
      ...
      //qr.rs/q/59827
      //https://www.codeqrcode.com/img_qr_urls/59827.png
      //  $image = 'https://www.codeqrcode.com/img_qr_urls/'.$type.'.png';
        $current_uri = 'http://qr.rs/q/'.$type;
        $content = urlencode($current_uri);
        $image = 'https://chart.googleapis.com/chart?chs=' . $img_size . 'x' . $img_size . '&cht=qr&chld=H|1&chl=' . $content;
...
<?php
    ...
    if(get_option('codeQRCodeShadow')){
        $style='border: 1px solid #676767; box-shadow: 7px 7px 5px #888888;';
    }else{
        $style='';
    }


    $output = '<div style="float:'.$align.'; width: '. $img_size .'px;'.$style.'" class="qrCodeTable"><table>';
        ...
        if(get_option('codeQRCodeSingleWidgetTitle') !="" && $called_from =="") {
            $output.='<tr style="border: none;"><td>' . get_option('codeQRCodeSingleWidgetTitle') . '</td></tr>';
        }

        //Img section
        $output.='<tr style="border: none;"><td>' .
            '<img style="margin: auto; " id="qr_code_generator_wprhe" src="' . $image . '" alt="Scan the QR Code" width="' . $img_size . '" height="' . $img_size . '" ' . $align . ' />' .
            '</td></tr>';

         // We need to check if PowerBy is checked in plugin settings
        if(get_option('codeQRCodePoweredBy') !=""){
            $output.= ' <tr> <td style="padding-top: 0; padding-bottom: 3px">' .
                '<a href="https://www.codeqrcode.com" target="blank" ><img style="margin: auto; max-width: 100px" src="'.plugins_url('images/made_with_love.png', __FILE__ ).'" border="0" alt="QR Code Generator"></a>' .
                ' </td></tr>';
        }
...
class Wp_widget_codeQRCode extends WP_Widget {
    ...
    public $widget_data_QR;

    public function __construct() {
        // widget actual processes
        parent::__construct(
            'wp_widget_codeQRCode', // Base ID
            'codeQRCode widget - QR Code', // Name
            array( 'description' => __( 'Display QR Codes in Sidebar')) // Args
        );
        $this->widget_data_QR = get_option('codeQRCodeWidgets');
    }
    ...
    function form( $instance ) {
        ...
        if(!empty($this->widget_data_QR) || ($this->widget_data_QR->flag && !empty($this->widget_data_QR->data))): ?>
            ...
            <p>
                ...
                <select id="<?php echo $this->get_field_id('widget_id'); ?>" name="<?php echo $this->get_field_name('widget_id'); ?>">
                    <?php foreach ( $this->widget_data_QR->data as $item ): ?>
...
class Wp_widget_codeQRCode_adsense extends WP_Widget {
    ...
    function form( $instance ) {
        ...
        if (get_option('codeQRCodeAds') !== '') {
            ...
            array_push($ad_data, unserialize(serialize($item_add)));
        ...
        if(!empty($ad_data)): ?>
            ...
            <p>
                <!-- Select - dropdown -->
                <label for="<?php echo $this->get_field_id('ad_code'); ?>"><?php _e('Select AdSense code:','envirra-backend'); ?></label>
                <select id="<?php echo $this->get_field_id('ad_code'); ?>" name="<?php echo $this->get_field_name('ad_code'); ?>">
                    <?php foreach ( $ad_data as $item ): ?>
